...
check_autorestart_pid() {
  A_PID=$(pgrep -f "RestartService_b.sh")
}
...
Service_Mirai_Dice_bash() {
  if [[ ${release} == "centos" ]]; then
    ...
    chmod +x /etc/init.d/Mirai-Dice-B
    update-rc.d -f Mirai-Dice-B defaults
  fi
  echo -e "${Info} Mirai-Dice 分身管理脚本下载完成 !"
}

# 下载进程守护脚本
Service_Mirai_AutoRestart() {
  if [[ -e ${file}/RestartService_b.sh ]]; then
    ...
    wget --no-check-certificate "https://raw.githubusercontent.com/rhwong/Dice_Linux_install/master/AutoRestart/RestartService_b.sh" -O ${file}/RestartService_b.sh; 
    chmod +x ${file}/RestartService_b.sh
    echo -e "${Info} Mirai-Dice 管理脚本下载完成 ! (注意：因为更新方式是直接覆盖，如果守护正在运行可能出现不可预料的错误。)"
  fi
    wget --no-check-certificate "https://raw.githubusercontent.com/rhwong/Dice_Linux_install/master/AutoRestart/RestartService_b.sh" -O ${file}/RestartService.sh; 
...
# 升级脚本
Update_Shell() {
  chmod +x MiraiDice_B.sh
  sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/rhwong/Dice_Linux_install/master/MiraiDice_B.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | head -1)
  [[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 下载源 !" && exit 0
  if [[ -e "/etc/init.d/Mirai-Dice-B" ]]; then
    rm -rf /etc/init.d/Mirai-Dice-B
    Service_Mirai_Dice_bash
  fi
  wget -N --no-check-certificate "https://raw.githubusercontent.com/rhwong/Dice_Linux_install/master/MiraiDice_B.sh" -O /root/MiraiDice_B.sh;
  chmod +x MiraiDice_B.sh
...
Start_Mirai_AutoRestart() {
  ...
  /etc/init.d/Mirai-Dice-B startauto
}
...
Stop_Mirai_AutoRestart() {
  ...
  /etc/init.d/Mirai-Dice-B stopauto
}

# 重启进程守护
Restart_Mirai_AutoRestart() {
  check_autorestart_pid
  [[ -n ${A_PID} ]] && /etc/init.d/Mirai-Dice-B stopauto
  /etc/init.d/Mirai-Dice-B startauto
}
...
if [[ "${action}" == "clearall" ]]; then
  ...
  echo && echo -e "  Dice! 分身B管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
